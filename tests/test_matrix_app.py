"""The seam-matrix Streamlit app (M1-06) actually loads and runs — the gate.
...
the saved run — FAILS the build. This is the gate that was missing twice before; it
ships with the screen.
"""
...
_APP = str(
    Path(__file__).resolve().parents[1] / "src" / "keystone" / "ui" / "matrix_app.py"
)
...
def test_app_loads_and_runs_without_exception() -> None:
    at = _app().run()
    # An import error in matrix_app.py (or any render exception) surfaces here.
...
def test_replay_of_an_incompatible_run_degrades_without_crashing(
    ...
    # and the honest empty state. Proves the gate catches a forced break (then the real
