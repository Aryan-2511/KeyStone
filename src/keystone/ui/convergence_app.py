"""The convergence hero as a Streamlit page (M2-0n).

    streamlit run src/keystone/ui/convergence_app.py

Renders `keystone.ui.convergence_screen` from the `keystone.demo` RunResult contract (the
convergence block, derived from REGISTERED_MAPPINGS) — the ONLY data source. Two modes:
...
state. Embedded via `components.v1.html` (an iframe), NEVER `st.html`.
"""
...
def render() -> None:
    st.set_page_config(
        page_title="Keystone — convergence", page_icon="🔗", layout="wide"
    )
