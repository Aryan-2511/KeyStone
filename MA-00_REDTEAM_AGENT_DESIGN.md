...
- **Memo-blind boundary (sacred):** the agent lives on the OFFENSE side. It must NEVER feed the attack channel / raw memo to the L1 detector. The 4 structural lock points named in the probe (`framework.py:137`, `project_financial`, etc.) must not loosen. An agent "reading the attack to be smarter" that touches detection = the convergence thesis collapses. Test: independence invariants still hold with the agent in the loop.
  - **Since ISO 20022 Phase 1 (`ADR-0036`):** `project_financial` no longer strips a hard-coded `memo` — it blanks every field in `keystone.core.transactions.UNTRUSTED_CHANNELS`, unconditionally and regardless of the pair's declared `AttackChannel`. This is a **generalization, not a loosening**: the lock strips MORE, never less, and with the registry at `{"memo"}` the memo strip is preserved exactly (behaviour-preserving — `recorded_run.json` unchanged). The lock point to hold is now the registry plus the unconditional strip; loosening = removing a name from `UNTRUSTED_CHANNELS`, or making the strip conditional on `channel`.
